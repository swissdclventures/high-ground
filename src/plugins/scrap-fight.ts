/**
 * Scrap fight view: one opponent, your eyes, one marker.
 *
 ...
 * glows blue→red. Bars, verdicts and controls are 2-D UI (scrap-fight-ui.tsx).
 *
...
import { executeScrapHitKnock, tickScrapHitKnock } from "./scrap-knock";
...
let npcSwingCount = 0;
/** Camera-projected version of the NPC's seeded front kick. */
...
/** Shape-match pull built automatically while an attack is held. */
let alignValue = 0;
...
/**
 * Thin, bruised-purple scratch at the side of the frame. This is deliberately
 * unlike the square attack language and unlike the thick blue/red glove glow.
 */
...
/**
 * Bout lock: jump/glide off, walk stays on so you can still move the world.
 * KO lock: freeze locomotion — any WASD cancels `triggerSceneEmote`, which is
 * why a lost KO used to look like you never fell (death clip starts, one step
 * aborts it, body pops back to idle in the wide shot).
 */
function setInputLock(lock: ScrapLock): void {
  if (lock === inputLock) return;
  inputLock = lock;
  if (lock === "none") {
    if (InputModifier.has(engine.PlayerEntity)) InputModifier.deleteFrom(engine.PlayerEntity);
    return;
  }
  const freeze = lock === "ko";
...
function poseGlove(entity: Entity, isRight: boolean, nowMs: number, defendHands: ScrapDefendHands): void {
  ...
  const lean = fight ? scrapDodgeLean(fight, nowMs) : 0;
  const side = fight?.dodgeDir === "right" ? 1 : -1;
  // Hold-to-block raises the chosen glove(s).
  const guard =
 ...
 * conceptual center match toward it; that same value now drives the visible
 * bottom-to-top fill instead of rendering a second shape.
 */
...
/**
 * One square, only in punch range. The existing solid square is now the
 * bottom-anchored fill inside the existing outline: dim wait, amber close,
 * gold opening, fully emissive green MAX. It disappears during defense.
 */
...
function paintImpact(nowMs: number): void {
  ...
  if (burst.alpha <= 0) {
    ...
    if (impactTf) impactTf.scale = HIDDEN;
    // A hit on YOU: three fine bruise scratches sit at the edge it came from.
    const camTf = Transform.getOrNull(rig.cam);
    if (camTf && hurtTf) {
...
function tickFollowupContact(nowMs: number): void {
  ...
  impactAt = nowMs;
  // This is THEIR contact: flash, lunge and thud are enough. Camera shake is
  // reserved exclusively for damage received by the player.
  lunge(0.24);
...
export function ensureScrapFightSystem(): void {
  ...
  // F is the KICK now. Walking out is [4] or the on-screen LEAVE button —
  // deliberately off the combat fingers, because a mis-hit exit ends the bout
...
function syncScrapFightUnsafe(input: {
  ...
  // 0. Charge automatically fills the target square; Shift raises the gloves;
  ...
  for (const ev of ticked.events) {
    ...
    if (ev.kind === "swing_start") {
      ...
      // Where it came from: keep received-hit scratches at the left/right edge,
      // never in the central square where the player reads attack timing.
  ...
  if (fight.outcome !== "pending" && !pendingResult) {
    if (fight.outcome === "player_win") {
      ...
      // Explicit F and automatic escape both release the planner immediately.
